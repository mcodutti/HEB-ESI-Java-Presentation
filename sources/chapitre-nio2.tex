...
\textbf{Préalable} NIO.2, comme présenté, concerne le JDK 7 (et suivants)
\end{frame}

\subsection{La classe <<Path>>}
\leconwithtocinside 

\begin{frame}[fragile]{La classe <<Path>>}
\begin{itemize}
\item Un \emph{fichier} est identifié par 
	\begin{itemize}
		\item son chemin à travers le \textit{filesystem}, 
		\item son nom complètement qualifié (FQN - \textit{Fully Qualified Name}), 
		\item son \textit{\textbf{Path}}
	\end{itemize}
\item Par exemple 
	\begin{itemize}
...
\begin{frame}[fragile]{La classe <<Path>>}
\begin{itemize}
\item La classe \emph{\texttt{Path}} en java représente un 
chemin (\textit{path}) et permet de le manipuler
	\begin{itemize}
		...
		\item ...
	\end{itemize}
\item La classe \texttt{\emph{Paths}}, 
...
\end{itemize}
\item \begin{Java} 	
Path path = Paths.get(args[0]), fqnPath = null;
try {
...
\item Une même méthode \texttt{foo} peut-être appelée 
\begin{Java} 
foo("one");
foo("one","two");
...
\begin{itemize}
\item La classe \emph{\texttt{Files}} est la deuxième classe utilitaire importante du \textit{package} NIO.2
\item Vérifier (\textit{checking}) un fichier ou un répertoire
	\begin{itemize}
	\item \textbf{\texttt{exists(Path, LinkOption)}} teste l'existence
	\begin{Java} 
	Files.exists(path);
	Files.exists(path, LinkOption.NOFOLLOW_LINKS));
	\end{Java}
	\item \textbf{\texttt{notExists(Path, LinkOption)}} teste la non-existence 
	\begin{Java} 
...
\begin{frame}[fragile]{Où l'on reparle de lire / écrire des fichiers}
\begin{itemize}
	\item D'autres méthodes encore de la classe \texttt{\emph{Files}}
	\begin{itemize}
...
\begin{frame}[fragile]{Où l'on reparle de lire / écrire des fichiers}
\begin{itemize}
\item D'autres méthodes encore de la classe \texttt{\emph{Files}} (suite)
	\begin{itemize}
