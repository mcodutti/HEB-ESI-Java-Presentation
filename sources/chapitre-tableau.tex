...
\par\medskip Si plusieurs dimensions
    \begin{itemize}
    \item Taille (potentiellement) différente d'un élément à l'autre
    \item $\Rightarrow$ Spécifier l'élément
    \item Exemple :
      \begin{Java}
  int[][]t = {{1,2},{2,3,4}};
...
\begin{itemize}
\item Si l'indice n'est pas valide (par rapport à la taille du tableau), la \sigle{JVM} lance une exception (\emph{IndexOutOfBoundException})
\item \emph{Exemple} :
\begin{Java}
  int[] entiers = {7,14,0};
