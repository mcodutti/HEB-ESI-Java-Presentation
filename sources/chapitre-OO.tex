...
\end{small}
\\\bigskip
\textit{En pratique, on rencontrera des situations intermédiaires}
\end{frame}

\begin{frame}{Visibilité des membres}
En \sigle{Java} : 4 visibilités
\begin{description}
  \item[public] : visible dans \textbf{toutes} les classes
  \item[privé] : n'est accessible que de \textbf{la} classe 
  \item[<<paqueté>>] : (pas de mot clé)  visible dans toutes les classes du \textit{package}
  \item[protégé] : (pas vu en 1ère année)
\end{description}
\bigskip
...
\\$\longrightarrow$ Différence entre déclaration et création 
\end{frame}

\begin{frame}[fragile]{Instanciation}
\emph{Exemple} : détaillons l'instanciation d'un étudiant
\begin{Java}
    Etudiant ada;  // référence créée sur la pile
\end{Java}
\begin{center}\includegraphics[scale=.5]{../img/java-oo-alloc1}\end{center}
\begin{Java}
...
\begin{frame}[fragile]{Comprendre <<static>>}
  ...
  \begin{itemize}
    \item Elle ne possède ni ne peut modifier les attributs de l'objet
...
\end{Java}
\java|Objetcs.equals| fait la même chose en plus court
\begin{Java}
