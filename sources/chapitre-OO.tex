...
\begin{frame}[fragile]{Comprendre <<static>>}
  ...
  int lg = outil.abs(-3);
  \end{Java}
  Pourquoi peut-on créer un \code{outil} ?
...
\begin{Java}[basicstyle=\scriptsize]
    public void afficher() {
        ...
        for( int i=0; i<nbEtudiants; i++ ) {
            System.out.print( étudiants[i].getNuméro() 
                     + ", " + étudiants[i].getNom() 
                     + ", " + étudiants[i].getAnnée() 
                     );
        }
    }
...
\begin{frame}[fragile]{Représentation textuelle}
On peut récrire la méthode \java|toString|
\\\bigskip
\emph{Exemple} :
\begin{Java}
...
\begin{frame}[fragile]{La méthode hashCode()}
La méthode \java|Objects.hash(Object... values)| 
\\crée un code à partir des paramètres fournis
\begin{itemize}
\item Si un attribut sert d'identifiant, donner celui-là
\item Sinon, donner un ensemble d'attributs avec des valeurs fort différentes d'un objet à l'autre
\end{itemize}
\medskip
\emph{Exemple} : Pour un étudiant
\begin{Java}
  public int hashCode() {
    return Objects.hash(this.numéro);
  }
\end{Java}
\end{frame}
